...
%
    ...
    % RGB Histogram 
    % TODO:Build buckets
    bucket = [0,0,0; 0,0,255; 0,255,255; 255,0,0; 255,255,0;255,0,255; 0,255,0; 255,255,255];
    pixel = [];
    RBGpixel = [];
    distance = [];
%     
%     % TODO:Place each pixel into a bucket
    fHistogramRGB = zeros(1, size(bucket,1));
    for y=1:size(imgDownSampled,1)
        for x=1:size(imgDownSampled,2)
            pixel = imgDownSampled(y,x,:);
            RGBpixel = [pixel(:,:,1),pixel(:,:,2),pixel(:,:,3)];
            for bucketIndex=1:size(bucket,1)   
                dist = norm((RGBpixel - bucket(bucketIndex,:)),2);
                distance = [distance,dist];
            end
            [dist_array,index] = sort(distance);
            fHistogramRGB(:,index(1)) = fHistogramRGB(:,index(1))+1;
            distance = [];
         end        
    end
...
%     RGB Area
    ...
    fAreaRGB = [];
    for y = 1:RGB_AREA_GRID_H
        for x = 1:RGB_AREA_GRID_W
            pixel = imgAreaRGB(y,x,1:3);
            fAreaRGB((y-1)*4+x,1) = pixel(1); 
            fAreaRGB((y-1)*4+x,2) = pixel(2); 
            fAreaRGB((y-1)*4+x,3) = pixel(3); 
        end
    end   
%     fAreaRGB = reshape(fAreaRGB,RGB_AREA_GRID_W*RGB_AREA_GRID_H*3,1);
    fAreaRGB = sum(fAreaRGB,2);
